...
2>nul (
   >>%OUTFILE% (call )
) && ((start /b "Title" "%PYTHON%" "Wrye Flash Launcher.pyw" -d %1 %2 %3 %4 %5 %6 %7 %8 %9 >>%OUTFILE% 2>&1) || echo.) || (echo Cannot launch Wrye Flash because another instance is already running) && PAUSE
